...
let personRekognitionData = rekognitionData.Labels.filter(
  (label) => label.Name === "Person"
);
...
(async function () {
  try {
    ...
    for (let index = 0; index < pageCount; index++) {
      ...
      const docPage = pdfData.Blocks.filter((b) => b.BlockType === "PAGE")[
        index
      ];
      const childs = [];
          ...
          personalInfo.forEach((pi) => {
            if (c[0].Text.toLowerCase().includes(pi.toLowerCase())) {
              ...
              ctx.fillRect(
                ...
                c[0].Geometry.BoundingBox.Height *
              ...
              ctx.fillText(
                redactedText,
                c[0].Geometry.BoundingBox.Left * canvasAndContext.canvas.width +
                  (c[0].Geometry.BoundingBox.Width *
                    canvasAndContext.canvas.width) /
                    2 -
                  (c[0].Geometry.BoundingBox.Width *
                    canvasAndContext.canvas.width -
                    ctx.measureText(redactedText).width) /
                    2,
                c[0].Geometry.BoundingBox.Top * canvasAndContext.canvas.height +
                  (c[0].Geometry.BoundingBox.Height *
                    canvasAndContext.canvas.height) /
      ...
      if (personRekognitionData && index === 0) {
        if (
          personRekognitionData.Instances &&
          personRekognitionData.Instances.length &&
          personRekognitionData.Instances[0].BoundingBox
        ) {
          const redactionImage = await Canvas.loadImage(
            __dirname + "/assets/images/redactedImage.jpg"
          );

          ctx.beginPath();
          ctx.drawImage(
            redactionImage,
            personRekognitionData.Instances[0].BoundingBox.Left *
              canvasAndContext.canvas.width,
            personRekognitionData.Instances[0].BoundingBox.Top *
              canvasAndContext.canvas.height,
            personRekognitionData.Instances[0].BoundingBox.Width *
              canvasAndContext.canvas.width,
            personRekognitionData.Instances[0].BoundingBox.Height *
              canvasAndContext.canvas.height
          );
        }
      }
      ...
      fs.writeFileSync(
        `redacted-advanced-hq-image-${index}`,
        pdfBuffer,
        "binary"
      );
      fs.writeFile(
        `redacted-advanced-hq-image-${index}.png`,
        image,
        function (error) {
          if (error) {
    ...
    (async () => {
      for (let index = 0; index < pageCount; index++) {
        await merger.add(`redacted-advanced-hq-image-${index}`);
      }
      await merger.save("merged-advanced-hq-image.pdf"); //save under given name and reset the internal document

      for (let index = 0; index < pageCount; index++) {
        fs.unlinkSync(`redacted-advanced-hq-image-${index}`);
      }
