...
(async function () {
  try {
    ...
    childs.forEach(c => {
        ...
        personalInfo.forEach(pi => {
            if (c[0].Text.toLowerCase().includes(pi.toLowerCase())) {
                ...
                ctx.strokeText(
                    redactedText,
                    ((c[0].Geometry.BoundingBox.Left * canvasAndContext.canvas.width) + ((c[0].Geometry.BoundingBox.Width * canvasAndContext.canvas.width)/2)),
    ...
    fs.writeFileSync(`redacted-advanced-${index}`, pdfBuffer,'binary');
    fs.writeFile(`redacted-advanced-${index}.png`, image, function (error) {
      if (error) {
    ...
    (async () => {
      for (let index = 0; index < pageCount; index++) {
        await merger.add(`redacted-advanced-${index}`);
      }
      await merger.save('merged-advanced.pdf'); //save under given name and reset the internal document

      for (let index = 0; index < pageCount; index++) {
        fs.unlinkSync(`redacted-advanced-${index}`);
      }
